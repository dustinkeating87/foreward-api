...
**Last verified:** 2026-05-20 (Wave 1 — picker pipeline + courses.json source-of-truth)
...
Shipped a four-task block to eliminate course-key drift between scraper, API, and frontend picker. Root problem: Lovable's hardcoded src/lib/courses.ts contained invented course keys (cardinal-red, sunny-lea, kleinburg, castlemore, etc.) and re-introduced private clubs we'd explicitly removed from the scraper. New alerts created via that picker referenced keys the scraper didn't recognize.

Architecture decision: extend the Phase 1 STATE.md auto-refresh pattern to a third consumer (the frontend). Scraper code → refresh_state.py → docs/courses.json (autogenerated) → GET /courses (API) → useCourses() hook (Lovable). Single source of truth, end-to-end.

Tasks shipped:

1. refresh_state.py emits docs/courses.json alongside STATE.md. Sorted alphabetically by name. 115 entries (54 GolfNow, 55 Chronogolf, 6 GTG). Both files regenerate on every push that triggers refresh-state.yml. Commit e0b78ed (+ workflow autocommit 5adc530).

2. GET /courses endpoint added to foreward-api. Public, no auth, 5-min browser cache. Loads docs/courses.json at module import; returns 503 if missing (no silent empty array). CORS already permissive (allow_origins=["*"]). Commit 79b74a8.

3. Lovable picker swapped to fetch from API. New src/hooks/useCourses.ts (plain useEffect + useState pattern matching existing Dashboard.tsx). CoursePicker accepts courses as props from CreateAlert. Loading + retry states. Picker now renders canonical scraper keys. Mid-task conflict detected — Lovable had pushed e539627 between prompt and execution; rebase aborted, work rebuilt on actual remote state. Commit 8451618.

4. Audit of alert_profiles for broken course keys. Root cause uncovered: the old picker was writing display LABELS (e.g. "Lakeview Golf Course") into the courses array instead of canonical slugs (e.g. "lakeview"). 28 affected alerts, all 100% broken (no valid keys), 9 distinct broken strings, all with unambiguous canonical equivalents. Single transaction SQL migration applied: array_replace() per mapping, all 9 mappings, 28 rows updated. Verified post-fix: zero broken strings remain. Active alert e7b90fc5 now has corrected keys.

Parser fix (separate but in scope): refresh_state.py was reading src/lib/courses.ts to determine "frontend picker contents." After Task 3, that file is no longer the picker's data source — useCourses.ts fetches from the API. Parser was stale and continued reporting bogus gaps even after Task 3 shipped. Fix: parser now reads docs/courses.json directly (the canonical API-driven picker source). by_request_frontend becomes empty. Commit b1c832b. STATE.md headline now shows 115 courses exposed in picker, 0 active-platform gap, 0 frontend-only entries.

Diagnostic findings (informational — not part of the shipped fix):

The picker-bug "ghost alerts" framing turned out to be more nuanced than initial audit suggested. Of the 28 broken alerts, 22 had matching sent_slots rows (159 SMS sent successfully over Apr 23 – May 14). The scraper had apparently been matching display labels successfully throughout the window. The 6 alerts with zero sent_slots are explained by date-range issues (narrow windows that didn't match, malformed date_to < date_from, or future date_from), not by the key mismatch. The picker bug was real, but its user-facing impact was smaller than the initial audit framing suggested. Net result: no user got fewer alerts than they should have; data is now normalized for future operations.

Live state movement (2026-05-20):

- 28 alert_profiles rows updated with canonical course keys
- 1 currently-active alert (e7b90fc5) now correctly keyed; will fire when its date_from of 2026-06-13 enters scraper lookahead window
- STATE.md gap counts: active-platform gap 49 → 0, frontend-only 43 → 0

Unresolved (filed as new ClickUp ticket): scraper appears to poll platforms every cycle regardless of whether the alert's date_from is within the platform's booking-release window. Active alert e7b90fc5 (date_from = June 13) triggers full GolfNow polls every 60s despite zero match potential. See known issue #36 below and ClickUp ticket for investigation plan. Pre-launch blocker for free-tier rollout.

Open ClickUp tickets after this block:
- 86ahbkwf6 — Lovable signup wall blocking free-tier reachability (pre-launch)
- 86ahbkw5h — Dashboard alert toggle reflects legacy active column, not status
- 86ahbkwka — Courses endpoint returns human-readable names (NOTE: no longer accurate after Tasks 1-3; verify and close)
- 86ahbkx68 — Pre-launch checklist: ≥5 paid alerts across ≥3 courses before FREE_TIER_ENABLED=true
- 86ahbkw2n — "2 renewals remaining" copy mismatch
- [new ticket to be filed today] — Scraper date-window polling bug
